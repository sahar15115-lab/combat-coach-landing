...
- ☐ **Step 2 — אפליקציה → קרוסלת-סווייפ** (4 מסכי-טלפון), מוחק את התלות האחרונה ב-`.mpanel/.pin-active` + סיכון-הגילוי (3 מסכים חבויים מאחורי כפתור-פנימי).
- ☐ **Step 3 — מחיקת קוד מת** (בלוקי `.pin-active` 581-598, פוזיציות טקסט nth-child, `.mswipe`, `#scrollCue`, `.msnap` שאריתי, snap-align אינרטי).
- ☐ **Step 4 — הרפיית `height:100svh`** לכל פאנל → גובה-תוכן ממורכז (19→~7-9 מסכים; ההזזה #1 של "טבעי").
- ☐ **Step 5 — ניקוי + טסט webview** (`page.route('**/gsap*',abort)` — הכיסוי שהיה מונע את הבאג) + עדכון טסטים ישנים.
